...
public final class ClientRuntime {
    ...
    private static final ResourceLocation HUD_LAYER = ResourceLocation.fromNamespaceAndPath(IdleCinematics.MOD_ID, "afk_status");
    private static final Set<ResourceLocation> HIDDEN_VANILLA_LAYERS = Set.of(VanillaGuiLayers.CROSSHAIR, VanillaGuiLayers.HOTBAR,
            VanillaGuiLayers.PLAYER_HEALTH, VanillaGuiLayers.ARMOR_LEVEL, VanillaGuiLayers.FOOD_LEVEL,
            VanillaGuiLayers.VEHICLE_HEALTH, VanillaGuiLayers.AIR_LEVEL, VanillaGuiLayers.EXPERIENCE_BAR,
            VanillaGuiLayers.EXPERIENCE_LEVEL, VanillaGuiLayers.JUMP_METER, VanillaGuiLayers.SELECTED_ITEM_NAME,
    ...
    @SubscribeEvent static void registerKeys(RegisterKeyMappingsEvent event) {
        event.register(TOGGLE); event.register(FORCE); event.register(SETTINGS);
    }
    ...
    @SubscribeEvent static void onKey(InputEvent.Key event) {
        if (event.getAction() == InputConstants.PRESS && !FORCE.matches(event.getKey(), event.getScanCode())
                && !TOGGLE.matches(event.getKey(), event.getScanCode()) && !SETTINGS.matches(event.getKey(), event.getScanCode())) activity(Minecraft.getInstance());
    }
    ...
    private static void renderIdleLayer(GuiGraphics graphics, net.minecraft.client.DeltaTracker deltaTracker) {
        Minecraft minecraft = Minecraft.getInstance();
        if (ACTIVITY.mode() == ActivityState.Mode.COUNTDOWN) {
            ...
            graphics.drawCenteredString(minecraft.font, Component.translatable("idlecinematics.countdown", seconds),
    ...
    private static void drawAnchored(GuiGraphics graphics, Minecraft minecraft, String text) {
        float scale = ClientConfig.HUD_SCALE.get().floatValue();
        graphics.pose().pushPose(); graphics.pose().scale(scale, scale, 1.0f);
        ...
        for (int index = 0; index < lines.length; index++) graphics.drawString(minecraft.font, Component.literal(lines[index]), x, y + index * 10, 0xFFFFFFFF, true);
        graphics.pose().popPose();
    }
