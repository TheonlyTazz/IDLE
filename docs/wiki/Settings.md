...
IDLE's configuration uses the same full-screen hierarchy as Minecraft's vanilla options:
...
3. **Done** in a category returns to the hub without saving yet.
4. **Done** on the hub validates, saves, and applies the complete settings draft.

Pressing Escape from a category returns to the hub. Pressing Escape from the hub cancels the draft and restores
the settings that were live when the screen opened. **Reset All** changes only the draft until the hub's **Done**
button is pressed.
